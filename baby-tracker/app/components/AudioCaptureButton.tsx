...
const AudioCaptureButton: React.FC<AudioCaptureButtonProps> = ({ onClick, isRecording }) => {
  return (
    <div className="flex justify-center items-center mx-2 ">
      <div className={`${isRecording ? 'w-[75px] h-[75px]' : 'w-[55px] h-[55px]'} relative cursor-pointer animate-wiggle`} tabIndex={0} style={{ filter: 'brightness(0.8)' }} onClick={onClick}>
        <div className="w-full h-full">
          <div className={`w-full h-full relative overflow-hidden text-center cursor-pointer rounded-full border border-transparent ${isRecording ? '' : 'bg-[radial-gradient(54.28%_54.28%_at_50%_49.91%,_rgba(0,0,0,0.11)_31.5%,_rgba(128,255,204,0.1)_83.01%,_rgba(128,255,204,0.26)_98%)]'} backdrop-blur-[23.5362px] shadow-[inset_-5.604px_-5.604px_280.193px_rgba(128,255,204,0.02)] transition-all duration-300`}>
            <div className="absolute z-10 w-full p-2 h-full flex items-center justify-center">
              {isRecording ? (
                <Image src="/images/audio-waves.png" alt="Recording Audio animation" width={75} height={75} className="w-auto h-auto" />
              ): (
